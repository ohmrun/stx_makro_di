...
class Injectors{
  static var printer = new haxe.macro.Printer();
    ...
    @:noUsing static public function run(fn:Expr):Expr {
      var self    = macro @:privateAccess stx.makro.di.Dependencies.instance.deferred.injector();
      ...
      function rec(t:haxe.macro.Type){
          return switch(t){
            case haxe.macro.Type.TFun(args, ret):
              ...
              var fn = macro function(di:stx.makro.di.DI) {
              ...
              var out = macro $access($v{id}, function(di) {
                var values = $fn(di);
                // stx.makro.di.Util.print('_run $id');
